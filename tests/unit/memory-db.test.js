...
describe('MemoryDB', () => {
  ...
  test('get() returns what was put()', async () => {
    ...
    await db.put('a', 'b', testData);
    const result = await db.get('a', 'b');
    expect(result).toBe(testData);
  });

  test('get() and put() works with buffers', async () => {
  ...
  test('get() with wrong secondary key returns undefined', async () => {
    const data = { foo: 'bar' };
    await db.put('a', 'b', data);
  ...
  test('query() returns all values for a secondary key', async () => {
  ...
  test('query() returns empty array for unknown primary key', async () => {
